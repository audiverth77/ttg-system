                ...
                <div class="bg-gray-200 bg-opacity-25 grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-8 p-6 lg:p-8 mb-4">
                    <div>
                        ...
                        <p class="mt-2 text-gray-500 text-sm">
                            Número Contacto: {{ $application->user->phone }}
                        </p>
                        <p class="text-gray-500 text-sm">
                            Email: {{ $application->user->email }}
                        </p>
                        <p class="text-gray-500 text-sm">
                            Direccion: {{ $application->user->address }}
                        </p>
                    </div>
                    <div class="flex gap-4 mt-4 justify-end">

                        <button class="abrir-modal-referencias inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 focus:bg-gray-700 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150" data-id="{{ $application->job->id }}">
                            Ver Referencias
                        </button>
                        <a target="_blank" href="{{ Storage::url( $application->user->cv ) }}">
